...
playingtime2 = ["games_subs","minutes_per_sub","unused_subs","points_per_match","on_goals_for","on_goals_against","plus_minus","plus_minus_per90","plus_minus_wowy","on_xg_for","on_xg_against","xg_plus_minus","xg_plus_minus_per90","xg_plus_minus_wowy"]
#miscallaneous(misc)
...
def get_tables(url, playing_time=False):
...
def frame_for_category(category,top,end,features):
    ...
    if category == 'playingtime':
        player_table = get_tables(url, True)
    else:
...
def get_outfield_data(season):

    top = 'https://fbref.com/en/comps/Big5/' + str(season - 1) + '-' + str(season) + '/'
    end = '/players/' + str(season - 1) + '-' + str(season) + '-Big-5-European-Leagues-Stats'
    ...
    df9 = frame_for_category('playingtime',top,end,playingtime2)

    df0 = np.array([season] * len(df9))
    df0 = pd.DataFrame({'season': df0})

    df = pd.concat([df0, df1, df2, df3, df4, df5, df6, df7, df8, df9], axis=1)
